 ...
 *
 * Multi-tienda: la tienda sale del request (headers que fija el proxy para
 * `/s/<slug>/…` o el subdominio), NUNCA de un dato del cliente.
 */
...
/** Tienda del request (o `null` si no existe / no está activa). */
async function currentStore(): Promise<PublicStore | null> {
  return (await getTenant()).store;
}
...
export async function checkCart(input: unknown): Promise<ActionResult<CartCheck>> {
  ...
  try {
    ...
    const fresh = await getFreshVariants(store.id, parsed.data.map((i) => i.variantId));
...
export async function applyCoupon(input: unknown): Promise<ActionResult<{ coupon: Coupon; message: string }>> {
  ...
  try {
    const store = await currentStore();
    if (!store) return fail(STORE_UNAVAILABLE);
    const [fresh, promotions] = await Promise.all([
      getFreshVariants(store.id, parsed.data.items.map((i) => i.variantId)),
    ...
    const result = await validateCouponServer(store.id, parsed.data.code, lines, base.merchandiseTotal, parsed.data.email);
...
async function quote(storeId: string, address: z.output<typeof addressSchema>, subtotal: number): Promise<ShippingQuoteResult> {
  const zones = await fetchShippingZonesFresh(storeId);
...
export async function quoteShippingAction(input: unknown): Promise<ActionResult<ShippingQuoteResult>> {
  ...
  try {
    const store = await currentStore();
    if (!store) return fail(STORE_UNAVAILABLE);
    return ok(await quote(store.id, parsed.data.address, parsed.data.subtotal));
  } catch (err) {
    console.error(err);
...
export async function createOrder(input: unknown): Promise<ActionResult<CreateOrderResult>> {
  ...
  try {
    ...
    if (data.couponCode) {
      const lines = base.lines.map((l) => ({ productId: l.productId, variantId: l.variantId, qty: l.qty, unitPrice: l.unitPrice }));
      const result = await validateCouponServer(store.id, data.couponCode, lines, base.merchandiseTotal, data.customer.email);
    ...
    if (data.fulfillment === "delivery" && data.address) {
      const q = await quote(store.id, data.address, withCoupon.merchandiseTotal);
      zone = q.zone;
      point = q.point;
...
export async function submitWithdrawal(_prev: WithdrawalState, formData: FormData): Promise<WithdrawalState> {
  ...
  try {
    const store = await currentStore();
    if (!store) return { status: "error", error: STORE_UNAVAILABLE, values };
    const supabase = createPublicClient();
