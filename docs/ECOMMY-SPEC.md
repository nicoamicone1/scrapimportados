...
- **Precios por cantidad (mayorista)** — `products.price_tiers jsonb not null default '[]'` (0021): `[{"min_qty": 6, "price": 900}, {"min_qty": 12, "price": 800}]` = "desde 6 unidades $ 900 c/u, desde 12 $ 800". Enteros `min_qty` ≥ 2 (hasta 999) estrictamente crecientes y precios > 0 estrictamente decrecientes (check `products_price_tiers_check` con `private.valid_price_tiers`; la base acepta hasta 10 tramos, el panel ofrece 4 y exige que cada precio sea menor al de la variante activa más barata). Plan: `pricing.tiers` (Starter en adelante).
  - **Regla por producto**: se suman las unidades de todas las variantes del producto en el carrito (3 S + 3 M = 6) y gana el tramo de mayor `min_qty` alcanzado. El precio del tramo **reemplaza** al de la variante sin subirlo nunca (una variante más barata que el tramo queda con su precio) y `compare_at_price` no participa.
  - **Orden**: tramo → promos por unidad (el % se calcula sobre el precio del tramo) → promos por cantidad (bonifican unidades a precio de tramo; si le ganan a una promo por unidad, la línea vuelve al tramo, no a la lista) → cupón → medio de pago. Motor: `tierPriceFor()` / `computeCart()` (`src/lib/pricing/tiers.ts`, `engine.ts`); SQL: `private.tier_price(product_id, qty, base)` y `create_order`, que calcula el piso de cada línea (`promo_floor_price`) sobre el precio del tramo y pasa ese precio como base a `compute_bundle_discount`.
  - **Pedido**: `order_items.unit_price` = unitario del tramo, `list_price` = el de la variante; el ahorro queda en `promo_total` (el resumen lo muestra como "Precio por cantidad", `CartTotals.tierDiscount`). Remito, mail y `/pedido/<token>` no cambian.
  - **Storefront**: tabla "Precio por cantidad" bajo el precio de la ficha (1–5 · 6–11 · 12 o más, resalta la cantidad elegida y el precio de arriba cambia con el selector), "Desde 6 u. $ X" en la card y "Precio mayorista desde 6 u." en cada línea del carrito.
  - **Sin 0021**: la columna no existe; las lecturas públicas reintentan sin ella (producto sin tramos, se cobra el precio de siempre) y el panel no deja guardar tramos ("falta la actualización 0021").
...
7. **Carritos abandonados** (migración 0020, plan `marketing.abandoned`: Starter en adelante; la tienda lo prende en Configuración › Pagos y checkout, `checkout.abandoned_reminders`; sólo se ofrece si la plataforma tiene `RESEND_API_KEY` y `SUPABASE_SERVICE_ROLE_KEY`). En "Tus datos", debajo del email, el tilde "Avisame por mail si dejo el pedido sin terminar" (nace **destildado**; ayuda: "Te mandamos un solo mail con tu carrito. Te podés dar de baja desde ese mail."). Sólo con el tilde, al continuar se guarda `checkout_sessions` (RPC `upsert_checkout_session` con `ip_hash` HMAC obligatorio; cupos 10 por email y tienda por día, 30 por IP, 500 por tienda, contados en `checkout_session_events`, que el visitante no puede borrar; nombre y precio de los ítems salen de la base) y el token queda en `localStorage` `ecommy:checkout:<storeId>`; los cambios del carrito la actualizan con debounce y destildar la borra (con reintentos si falla la red; una sesión ya avisada o dada de baja se vacía en vez de borrarse). Al confirmar, `mark_checkout_recovered` (mismo email y tienda; `create_order` no cambia). Las bajas viven en `checkout_unsubscribes` (por email y tienda, nadie las borra). El cron (`src/lib/email/abandoned-notices.ts`, diario; `/api/cron/abandoned` opcional con plan Pro de Vercel) manda UN mail por sesión entre 3 y 48 h sin tocar el carrito, con reclamo atómico (`claim_checkout_reminder`), tope 50 por tienda y corrida (10 en prueba) y 300 en total ("Dejaste tu pedido a mitad de camino": ítems con precio de hoy, total, "Terminar mi pedido" → `/carrito/recuperar/<token>`, que guarda el token en una cookie httpOnly de 10 minutos y redirige a `/carrito` sin dejar el token en la URL, reponiendo el carrito y omitiendo lo que ya no está; "No quiero recibir estos avisos" → `/carrito/recuperar/<token>?baja=1` con botón de confirmación, más `List-Unsubscribe` de un clic vía `POST /api/email/unsubscribe`). Si la persona compra hasta 7 días después del aviso, la sesión se marca recuperada. Bandeja de sólo lectura en `/admin/pedidos/abandonados` (pendiente / avisado / recuperado / baja / vencido). Sesiones y eventos se borran a los 30 días; la purga corre aunque no haya mails configurados.
