...
async function slugTaken(ctx: AdminContext, slug: string, exceptId?: string | null) {
  let q = ctx.supabase.from("products").select("id").eq("slug", slug).limit(1);
...
export async function saveProduct(input: unknown): Promise<ActionResult<{ product: AdminProductDetail }>> {
  return runAction(async () => {
    ...
    if (skus.length) {
      let q = supabase.from("product_variants").select("sku, product_id, products(name)").in("sku", skus);
    ...
    const row = {
      ...
      related_ids: [...new Set(data.related_ids)].filter((id) => id !== data.id),
      published_at: data.status === "active" ? (before?.published_at ?? now) : (before?.published_at ?? null),
    } satisfies TablesUpdate<"products">;

    let productId: string;
    if (before) {
      const { error } = await supabase.from("products").update(row).eq("id", before.id);
      if (error) throw new Error(error.message);
      productId = before.id;
    } else {
      const { data: created, error } = await supabase
        .from("products")
        .insert({ ...row, source: "manual" } satisfies TablesInsert<"products">)
        .select("id")
        .single();
      if (error) throw new Error(error.message);
    ...
    {
      let del = supabase.from("product_categories").delete().eq("product_id", productId);
      ...
      if (categoryIds.length) {
        const { error } = await supabase
          ...
          .upsert(
            categoryIds.map((category_id, position) => ({ product_id: productId, category_id, position })),
    ...
    const { data: imageRows } = await supabase.from("product_images").select("id").eq("product_id", productId);
    ...
    await Promise.all(
      changedCombos.map((v) =>
        supabase.from("product_variants").update({ option_values: { __tmp: v.id as string } }).eq("id", v.id as string),
      ),
    );
    ...
    if (fresh.length) {
      const { data: inserted, error } = await supabase
        .from("product_variants")
        .insert(fresh.map((v) => ({ ...variantRow(v, positions.get(v) ?? 0), product_id: productId, stock: 0 })))
        .select("id, option_values");
      if (error) throw new Error(error.message);
    ...
    if (before && before.slug !== slug) {
      await upsertRedirect(supabase, ctx.user.id, `/producto/${before.slug}`, `/producto/${slug}`);
    }
    ...
    revalidateProducts([slug, before?.slug], true);
...
export async function inlineUpdateVariant(
  ...
  return runAction(async () => {
    ...
    if (Object.keys(patch).length) {
      const { error } = await ctx.supabase.from("product_variants").update(patch).eq("id", v.id);
      if (error) throw new Error(error.message);
    }
...
export async function bulkProducts(input: unknown): Promise<ActionResult<{ count: number }>> {
  return runAction(async () => {
    ...
    const { data: rows } = await supabase.from("products").select("id, slug").in("id", ids);
    const found = rows ?? [];
    ...
    if (action === "publish" || action === "draft" || action === "archive" || action === "restore") {
      ...
      const { error } = await supabase.from("products").update({ status }).in("id", foundIds);
      if (error) throw new Error(error.message);
      if (status === "active") {
        await supabase.from("products").update({ published_at: new Date().toISOString() }).in("id", foundIds).is("published_at", null);
      }
    } else if (action === "add_category" && categoryId) {
      const { error } = await supabase
        .from("product_categories")
        .upsert(foundIds.map((product_id) => ({ product_id, category_id: categoryId, position: 99 })), {
          onConflict: "product_id,category_id",
          ignoreDuplicates: true,
        });
      if (error) throw new Error(error.message);
    } else if (action === "remove_category" && categoryId) {
      const { error } = await supabase.from("product_categories").delete().eq("category_id", categoryId).in("product_id", foundIds);
      if (error) throw new Error(error.message);
    }
...
export async function duplicateProduct(id: string, options: { images: boolean } = { images: true }): Promise<ActionResult<{ id: string }>> {
  return runAction(async () => {
    ...
    if (src.product_categories?.length) {
      await supabase
        .from("product_categories")
        .insert(src.product_categories.map((c) => ({ product_id: newId, category_id: c.category_id, position: c.position })));
    }
    ...
    if (parsed.data.images && src.product_images?.length) {
      const images = [...src.product_images].sort((a, b) => a.position - b.position);
      for (const img of images) {
        let url = img.url;
        const path = storagePathFromUrl(img.url);
        if (path) {
          const ext = path.split(".").pop() ?? "webp";
          const target = `products/${newId}/${nanoid(10)}.${ext}`;
        ...
        const { data: newImg } = await supabase
          .from("product_images")
          .insert({ product_id: newId, url, alt: img.alt, position: img.position, width: img.width, height: img.height })
          .select("id")
          .single();
    ...
    if (variants.length) {
      ...
      await supabase.from("product_variants").insert({ product_id: newId, title: "Default", price: 0, stock: 0 });
    }
...
export async function deleteProduct(id: string): Promise<ActionResult<{ id: string }>> {
  return runAction(async () => {
    ...
    const { data: referencing } = await supabase.from("products").select("id, related_ids").contains("related_ids", [p.id]);
    ...
    await supabase.from("redirects").delete().eq("to_path", `/producto/${p.slug}`);

    await removeMediaIfUnused(supabase, urls);
    // Archivos sueltos de la carpeta del producto.
    const { data: files } = await supabase.storage.from(MEDIA_BUCKET).list(`products/${p.id}`, { limit: 1000 });
    if (files?.length) await supabase.storage.from(MEDIA_BUCKET).remove(files.map((f) => `products/${p.id}/${f.name}`));
    ...
    revalidateProducts([p.slug], true);
    return ok({ id: p.id });
  });
}
...
export async function addProductImages(input: unknown): Promise<ActionResult<{ images: AdminImage[] }>> {
  return runAction(async () => {
    ...
    for (const img of images) {
      if (!img.path.startsWith(`products/${productId}/`) || storagePathFromUrl(img.url) !== img.path) {
        return fail("La imagen no corresponde a este producto.");
      }
    }
    const { data: p } = await supabase.from("products").select("id, slug, name").eq("id", productId).maybeSingle();
    if (!p) return fail("El producto ya no existe.");
...
export async function deleteProductImage(imageId: string): Promise<ActionResult<{ id: string }>> {
  return runAction(async () => {
    ...
    const { error } = await ctx.supabase.from("product_images").delete().eq("id", img.id);
    if (error) throw new Error(error.message);
    await removeMediaIfUnused(ctx.supabase, [img.url]);
    await logAudit(ctx, {
    ...
    revalidateProducts([img.products?.slug]);
    return ok({ id: img.id });
  });
}
  ...
  return runAction(async () => {
    ...
    const results = await Promise.all(
      parsed.data.ids.map((id, position) =>
        ctx.supabase.from("product_images").update({ position }).eq("id", id).eq("product_id", parsed.data.productId),
      ),
    );
    const failed = results.find((r) => r.error);
    if (failed?.error) throw new Error(failed.error.message);
    const { data: p } = await ctx.supabase.from("products").select("slug").eq("id", parsed.data.productId).maybeSingle();
    revalidateProducts([p?.slug]);
    return ok();
  });
}
...
export async function updateImageAlt(imageId: string, alt: string): Promise<ActionResult> {
  return runAction(async () => {
    ...
    const { data, error } = await ctx.supabase
      ...
      .maybeSingle();
    if (error) throw new Error(error.message);
    revalidateProducts([data?.products?.slug]);
    return ok();
  });
}
