...
export default async function WithdrawalPage({ searchParams }: PageProps<"/arrepentimiento">) {
  const [{ settings }, sp] = await Promise.all([getStoreDisplay(), searchParams]);
        ...
        <p className="mt-2 text-fg-muted">
          ...
          {settings.policies.returns_md ? (
            <>
              {" "}
              Más información en{" "}
              <Link href="/politicas/cambios-y-devoluciones" className="link">
