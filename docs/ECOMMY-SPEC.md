...
7. **Carritos abandonados** (migración 0020, plan `marketing.abandoned`: Starter en adelante; la tienda lo prende en Configuración › Pagos y checkout, `checkout.abandoned_reminders`). En "Tus datos", debajo del email, el tilde "Avisame por mail si dejo el pedido sin terminar" (nace **destildado**; ayuda: "Un solo aviso, sin ofertas inventadas. Podés darte de baja desde el mail."). Sólo con el tilde, al continuar se guarda `checkout_sessions` (RPC `upsert_checkout_session`, cupos 10 por email y tienda por día, 30 por IP, 500 por tienda; nombre y precio de los ítems salen de la base) y el token queda en `localStorage` `ecommy:checkout:<storeId>`; los cambios del carrito la actualizan con debounce y destildar la borra. Al confirmar, `mark_checkout_recovered` (mismo email y tienda; `create_order` no cambia). El cron (`src/lib/email/abandoned-notices.ts`, diario y cada 6 h) manda UN mail por sesión entre 3 y 48 h sin tocar el carrito ("Dejaste tu pedido a mitad de camino": ítems con precio de hoy, total, "Terminar mi pedido" → `/carrito?recuperar=<token>` que repone el carrito omitiendo lo que ya no está, y "No quiero recibir estos avisos" → `/carrito?baja=<token>`, baja por email y tienda con botón de confirmación). Bandeja de sólo lectura en `/admin/pedidos/abandonados` (pendiente / avisado / recuperado / baja). Las sesiones se borran a los 30 días.
