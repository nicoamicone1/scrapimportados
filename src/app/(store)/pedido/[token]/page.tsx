...
export default async function OrderPage({ params, searchParams }: PageProps<"/pedido/[token]">) {
  ...
  const url = absoluteUrl(`/pedido/${order.token}`);
    ...
    <div className="store-container max-w-[calc(880px+2*var(--gutter))] py-[var(--space-section-sm)]">
        ...
        <section className="mt-6 rounded-lg border border-border bg-surface p-5" aria-labelledby="pagar">
          ...
          {t.instructionsMd || method?.instructionsMd ? (
            <div
              ...
              dangerouslySetInnerHTML={{ __html: markdownToHtml(t.instructionsMd || method?.instructionsMd || "") }}
            />
          ) : null}
          {order.expiresAt ? (
      ...
      <footer className="mt-10 border-t border-border pt-6 text-sm">
        ...
        <p className="mt-6 text-xs text-fg-muted">
          ...
          <Link href={`/arrepentimiento?pedido=${order.number}`} className="link">
