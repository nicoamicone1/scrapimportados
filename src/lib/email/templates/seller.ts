...
import { formatDeadline, itemsBlock, moneyOf, platformBrand, totalsBlock } from "./shared";
...
export function newOrderSellerEmail(order: OrderEmailData, store: SellerStoreInfo): EmailContent {
  ...
  return renderEmail({
    subject,
    preheader: `${customer} · Pago: ${payment} · ${order.deliveryText}.`,
    brand: platformBrand(store.platformUrl),
    blocks: [
      { t: "heading", text: `Nuevo pedido #${order.number}` },
      { t: "p", content: [{ b: customer }, ` hizo un pedido en ${store.name} por `, { b: money(order.total) }, "."] },
      {
        t: "rows",
        rows: [
...
export function withdrawalSellerEmail(w: WithdrawalEmailData, store: SellerStoreInfo): EmailContent {
  ...
  return renderEmail({
    subject,
    preheader: `${w.name} pidió revocar una compra${w.orderNumber ? ` (pedido #${w.orderNumber})` : ""}.`,
    brand: platformBrand(store.platformUrl),
    blocks: [
      { t: "heading", text: "Nueva solicitud de arrepentimiento" },
      { t: "p", content: [{ b: w.name }, ` usó el botón de arrepentimiento de ${store.name} para revocar una compra.`] },
      {
        t: "rows",
        rows: [
      ...
      w.reason && { t: "p", content: ["Motivo: ", { b: w.reason }] },
